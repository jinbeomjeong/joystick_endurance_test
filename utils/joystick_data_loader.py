...
class JoystickDataset(Dataset):
    def __init__(self, joystick_data: np.array, seq_len: int = 1):
        self.__seq_len = seq_len
        self.__dataset_tensor = torch.FloatTensor(joystick_data)

    def __len__(self):
        return self.__dataset_tensor.shape[0] - self.__seq_len

    def __getitem__(self, idx):
        return (self.__dataset_tensor[idx: idx+self.__seq_len],
                self.__dataset_tensor[idx+self.__seq_len, 1])


class JoystickDataModule(pl.LightningDataModule):
    def __init__(self, dataset_path: str, seq_len: int = 1, batch_size: int = 32,
        ...
        #self.__raw_data_arr = self.__raw_data_arr[0:int(self.__raw_data_arr.shape[0]/64)]
        self.__raw_data_5_arr = self.__raw_data_5_arr[0:int(self.__raw_data_5_arr.shape[0]/20)]

    def setup(self, stage=None):
        self.__dataset = JoystickDataset(joystick_data=self.__raw_data_arr, seq_len=self.__seq_len)
